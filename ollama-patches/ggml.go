...
func (b *Backend) Load(ctx context.Context, progress func(float32)) error {
	...
	if dstorage.IsAvailable() && dstorage.IsCudaAvailable() {
		if dl, err := dstorage.NewLoader(0); err == nil {
			dsLoader = dl
			defer dsLoader.Close()
			slog.Info("DirectStorage: enabled for GPU tensor loading (SSD → GPU bypass)")
		} else {
			slog.Warn("DirectStorage: available but failed to create loader, falling back to standard I/O", "error", err)
		}
	}
