...
var (
	...
	procStreamToCudaBatch  *syscall.Proc
...
func loadDLL() error {
	...
	procs := []struct {
		...
		{"ds_loader_stream_to_cuda_batch", &procStreamToCudaBatch},
...
// StreamToCudaBatch loads multiple tensors from a single file to CUDA device pointers.
// Opens the file once, sizes staging buffer to largest tensor, processes all tensors
// with minimal DMA submits. This is much faster than calling StreamToCuda in a loop.
func (l *Loader) StreamToCudaBatch(filePath string, fileOffsets []uint64, sizes []uint64, cudaDestPtrs []uint64) error {
	if l.closed {
		return errors.New("loader is closed")
	}
	count := len(fileOffsets)
	if count == 0 {
		return nil
	}
	if len(sizes) != count || len(cudaDestPtrs) != count {
		return ErrInvalidArgument
	}
	...
	ret, _, _ := procStreamToCudaBatch.Call(
		l.handle,
		uintptr(unsafe.Pointer(widePath)),
		uintptr(unsafe.Pointer(&fileOffsets[0])),
		uintptr(unsafe.Pointer(&sizes[0])),
		uintptr(unsafe.Pointer(&cudaDestPtrs[0])),
		uintptr(count),
	)
	if int32(ret) != 0 {
		return fmt.Errorf("batch stream to CUDA failed: HRESULT=0x%08X", uint32(GetLastHResult()))
	}
	return nil
}
