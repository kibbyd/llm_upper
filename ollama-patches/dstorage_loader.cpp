...
// --- Batch stream: multiple tensors from one file in minimal DMA submits ---
int ds_loader_stream_to_cuda_batch(
    ...
    if (!loader || !file_path || !file_offsets || !sizes || !cuda_dest_ptrs || count == 0)
        return -1;
    if (!EnsureCudaLoaded()) { g_lastHR = E_FAIL; return -1; }

    // Find the largest tensor to size the staging buffer
    uint64_t maxSize = 0;
    ...
    // Ensure staging buffer fits the largest tensor
    if (!EnsureStagingBuffer(loader, maxSize)) return -1;
    ...
    // Create a persistent fence+event for this batch operation
    ComPtr<ID3D12Fence> fence;
    hr = loader->device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence));
    if (FAILED(hr)) { g_lastHR = hr; return -1; }
    HANDLE event = CreateEvent(NULL, FALSE, FALSE, NULL);
    if (!event) { g_lastHR = HRESULT_FROM_WIN32(GetLastError()); return -1; }
    UINT64 fenceVal = 0;

    g_cuCtxSetCurrent(g_cudaContext);

    for (uint64_t i = 0; i < count; i++) {
        if (sizes[i] == 0 || cuda_dest_ptrs[i] == 0) continue;

        // Enqueue DMA: file -> staging buffer (with chunking for >32MB)
        uint64_t remaining = sizes[i];
        uint64_t bufferOffset = 0;
        while (remaining > 0) {
            uint32_t chunkSize = (remaining > DS_MAX_CHUNK_SIZE)
                ? (uint32_t)DS_MAX_CHUNK_SIZE
            ...
            request.Options.DestinationType = DSTORAGE_REQUEST_DESTINATION_BUFFER;
            request.Source.File.Source = file.Get();
            request.Source.File.Offset = file_offsets[i] + bufferOffset;
            request.Source.File.Size = chunkSize;
            request.UncompressedSize = chunkSize;
            request.Destination.Buffer.Resource = loader->staging.resource.Get();
            request.Destination.Buffer.Offset = bufferOffset;
        ...
        // Submit + wait for this tensor's DMA
        fenceVal++;
        hr = fence->SetEventOnCompletion(fenceVal, event);
        if (FAILED(hr)) { CloseHandle(event); g_lastHR = hr; return -1; }
        loader->queue->EnqueueSignal(fence.Get(), fenceVal);
        loader->queue->Submit();
        WaitForSingleObject(event, INFINITE);
        ...
        // Device-to-device copy: staging -> destination
        CUresult cr = g_cuMemcpyDtoD(
            (CUdeviceptr)cuda_dest_ptrs[i],
            loader->staging.devPtr,
            (size_t)sizes[i]);
        if (cr != CUDA_SUCCESS) {
            CloseHandle(event);
            g_lastHR = (HRESULT)(0xCD000000 | (cr & 0xFFFF));
...
int ds_loader_stream_to_cuda_batch(DSLoaderHandle loader, const wchar_t* file_path,
                                    const uint64_t* file_offsets, const uint64_t* sizes,
                                    const uint64_t* cuda_dest_ptrs, uint64_t count) { return -1; }
