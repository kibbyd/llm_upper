...
func TestVMMAvailable(t *testing.T) {
	avail := VMMAvailable()
	t.Logf("CUDA VMM available: %v", avail)
	if avail {
		t.Log("CUDA VMM is available — can use virtual memory overcommit for MoE expert streaming")
	} else {
		t.Log("CUDA VMM not available — requires CUDA 10.2+ (driver 440.33+)")
	}
}

func TestVMMGranularity(t *testing.T) {
	if !VMMAvailable() {
		t.Skip("CUDA VMM not available")
	}

	granularity := VMMGetGranularity()
	t.Logf("VMM granularity: %d bytes (%d MB)", granularity, granularity/1024/1024)

	if granularity == 0 {
		t.Fatal("VMM granularity is 0 - this shouldn't happen if VMM is available")
	}
	if granularity < 1024*1024 {
		t.Logf("Warning: granularity is less than 1 MB, which is unusual")
	}
}

func TestVMMReserveMapUnmap(t *testing.T) {
	if !VMMAvailable() {
		t.Skip("CUDA VMM not available")
	}
	...
	granularity := VMMGetGranularity()
	if granularity == 0 {
		t.Fatal("Failed to get VMM granularity")
	}

	// Reserve 64 MB of virtual address space
	vaSize := uint64(64 * 1024 * 1024)
	// Round up to granularity
	vaSize = ((vaSize + granularity - 1) / granularity) * granularity

	vaPtr := VMMReserve(vaSize, granularity)
	if vaPtr == 0 {
		t.Fatalf("Failed to reserve %d bytes of VA space: HRESULT=0x%08X", vaSize, GetLastHResult())
	}
	t.Logf("Reserved %d MB of virtual address space at 0x%X", vaSize/1024/1024, vaPtr)

	// Create physical memory (just 1 granularity unit)
	physSize := granularity
	physHandle := VMMCreatePhysical(physSize)
	if physHandle == 0 {
		VMMFree(vaPtr, vaSize)
		t.Fatalf("Failed to create physical memory: HRESULT=0x%08X", GetLastHResult())
	}
	t.Logf("Created %d MB of physical memory (handle=0x%X)", physSize/1024/1024, physHandle)

	// Map physical to first part of VA range
	err := VMMMap(vaPtr, physSize, physHandle, 0)
	if err != nil {
		VMMReleasePhysical(physHandle)
		VMMFree(vaPtr, vaSize)
		t.Fatalf("Failed to map physical to VA: %v", err)
	}
	t.Logf("Mapped physical memory to VA 0x%X", vaPtr)

	// Unmap
	err = VMMUnmap(vaPtr, physSize)
	if err != nil {
		t.Fatalf("Failed to unmap: %v", err)
	}
	t.Log("Unmapped physical memory")

	// Map to a different part of VA range (demonstrate remapping)
	offsetVA := vaPtr + physSize // Second granularity-sized chunk
	err = VMMMap(offsetVA, physSize, physHandle, 0)
	if err != nil {
		VMMReleasePhysical(physHandle)
		VMMFree(vaPtr, vaSize)
		t.Fatalf("Failed to remap physical to different VA: %v", err)
	}
	t.Logf("Remapped same physical memory to different VA 0x%X", offsetVA)

	// Cleanup
	err = VMMUnmap(offsetVA, physSize)
	if err != nil {
		t.Fatalf("Failed to unmap before cleanup: %v", err)
	}

	err = VMMReleasePhysical(physHandle)
	if err != nil {
		t.Fatalf("Failed to release physical memory: %v", err)
	}
	t.Log("Released physical memory")

	err = VMMFree(vaPtr, vaSize)
	if err != nil {
		t.Fatalf("Failed to free VA space: %v", err)
	}
	t.Log("Freed virtual address space")
